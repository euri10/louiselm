...
local decode_ok, entries = pcall(nvim.json.decode, raw)
...
for _, entry in ipairs(entries) do
  local edit_ok, edit_err = pcall(nvim.cmd.edit, nvim.fn.fnameescape(entry.file))
  if not edit_ok then
  ...
  if not render_ok then
    fail(("TOhtml failed for %s [%d-%d]: %s"):format(entry.file, entry.first, entry.last, tostring(html)))
    return
  end

  nvim.fn.writefile(html, entry.out)
end
