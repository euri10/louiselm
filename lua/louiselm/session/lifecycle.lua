...
local function handle_notification(self, message)
  ...
  if message.method == "github.com/copilot/sessionEvent" then
    -- session/load may replay historical turn_start events; they are not live work.
    if self.state.status == "starting" or self.acp_session_id == nil then
      return
    end
    local running, activity_error = Validation.copilot_activity(message.params, self.acp_session_id)
