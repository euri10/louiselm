...
local SECTIONS = {
  ...
  },
  {
    ...
    emit = function(lines, context)
      ...
      append_prose(
        lines,
        "Hover the Agent name (for example, codex) in a chat window's winbar to display its full Agent-scoped Session ID without changing keyboard focus. The tooltip identifies the hovered window's Session and disappears when the pointer leaves the name or you type. Use :LouiselmSessionId to copy the focused Session's ID.",
        context
      )
      append_prose(
        lines,
        "Hover requires a terminal/GUI that reports mouse movement and enabled 'mouse'. Opening the interactive chat enables 'mousemoveevent'; disposing or reconfiguring the UI restores its previous value unless you changed it afterward. Set 'nomousemoveevent' to disable hover. No mouse mappings are replaced. Headless Session APIs do not change these options.",
        context
      )
      append_prose(
        lines,
