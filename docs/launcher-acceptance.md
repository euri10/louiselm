...
| 5 — measured fixed runtime | [`RuntimePackage::measure`](../skills-core/src/registry.rs) hashes the executable and listed adapters, retaining version/origin as registration metadata. `Registry::open_trusted` validates ownership, permissions and types throughout the entire mounted runtime tree (`louiselm-ln30`). Production validates the measured Bubblewrap path before preparation. | `louiselm-etth` removes the unverified library-baseline/policy-version declarations from registration and measurement; launch receipts separately bind the enforced isolation contract. Actual host/library conformance evidence remains `louiselm-d6fv.12`. Unlisted files are protected by root ownership, not added to the digest format; root compromise remains outside the boundary. Provider packaging/masking belongs to `louiselm-d6fv.3`. |
